...
class RencanaBibitExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths
{
    protected $request;
    ...
    public function collection()
    {
        ...
        // Apply filters
        if ($this->request->filled('kelompok')) {
            $query->where('id_kelompok', $this->request->kelompok);
        }
        ...
        if ($this->request->filled('search')) {
            $query->where('jenis_bibit', 'like', '%' . $this->request->search . '%');
        }
    ...
    public function map($bibit): array
    {
        ...
        return [
            ...
            $bibit->tinggi ? number_format($bibit->tinggi, 2, ',', '.') . ' cm' : '-',
