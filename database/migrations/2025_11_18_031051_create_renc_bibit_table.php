...
return new class extends Migration
{
    ...
    {
        Schema::create('renc_bibit', function (Blueprint $table) {
            $table->id('id_bibit');

            // ✅ Foreign key ke tabel kelompoks (BUKAN kelompok)
            // Pastikan nama tabel sesuai dengan yang ada di database
            $table->unsignedBigInteger('id_kelompok');
            $table->foreign('id_kelompok')
                  ->references('id')
                  ->on('kelompoks') // ✅ Sesuaikan dengan nama tabel yang benar
                  ->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('renc_bibit');
    }
};