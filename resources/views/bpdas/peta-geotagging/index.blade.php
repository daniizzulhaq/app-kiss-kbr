...
<div class="container mx-auto px-4 py-6 sm:py-8">
    ...
    <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 flex items-center gap-2">
            ...
            Verifikasi Peta Geotagging Kelompok
        </h1>
        <p class="text-gray-600 mt-2">Kelola dan verifikasi peta geotagging yang diupload oleh kelompok</p>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-blue-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl sm:text-3xl font-bold text-gray-800 mt-1">{{ \App\Models\PetaGeotagging::count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-yellow-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl sm:text-3xl font-bold text-yellow-600 mt-1">{{ \App\Models\PetaGeotagging::where('status', 'pending')->count() }}</p>
                </div>
                <span class="text-2xl sm:text-4xl">⏳</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-green-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl sm:text-3xl font-bold text-green-600 mt-1">{{ \App\Models\PetaGeotagging::where('status', 'diterima')->count() }}</p>
                </div>
                <span class="text-2xl sm:text-4xl">✅</span>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 border-l-4 border-red-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl sm:text-3xl font-bold text-red-600 mt-1">{{ \App\Models\PetaGeotagging::where('status', 'ditolak')->count() }}</p>
                </div>
                <span class="text-2xl sm:text-4xl">❌</span>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-6">
        <form method="GET" action="{{ route('bpdas.peta-geotagging.index') }}" class="flex flex-col sm:flex-row gap-4">
            <!-- Search -->
            <div class="flex-1">
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-green-600 to-green-700 px-4 sm:px-6 py-4">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-bold text-white flex items-center gap-2">
                    <span>📋</span>
                    Daftar Peta Geotagging
                </h3>
                @if($petaGeotaggings->total() > 0)
                <span class="text-sm text-green-100">
                    Menampilkan {{ $petaGeotaggings->firstItem() }} - {{ $petaGeotaggings->lastItem() }} dari {{ $petaGeotaggings->total() }} data
                </span>
                @endif
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($petaGeotaggings as $index => $peta)
                    <tr class="hover:bg-green-50 transition-colors">
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            {{ $petaGeotaggings->firstItem() + $index }}
                        </td>
                        ...
                        <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-center">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('bpdas.peta-geotagging.show', $peta) }}" 
                    ...
                    <tr>
                        <td colspan="7" class="px-4 sm:px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-sm text-gray-500 mb-6">Belum ada peta geotagging yang diupload kelompok.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <!-- Pagination -->
        @if($petaGeotaggings->hasPages())
        <div class="px-4 sm:px-6 py-4 border-t border-gray-200 bg-gray-50">
            {{ $petaGeotaggings->links() }}
        </div>
        @endif
    </div>
</div>
