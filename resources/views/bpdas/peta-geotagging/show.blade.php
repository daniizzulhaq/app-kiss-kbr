...
@section('title', 'Verifikasi Peta Geotagging - Sistem KBR')
...
<div class="container mx-auto px-4 py-6 sm:py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 flex items-center gap-2">
                    ...
                    Verifikasi Peta Geotagging
                </h1>
                <p class="text-gray-600 mt-2">Detail dan verifikasi peta geotagging dari kelompok</p>
            </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
                    <h3 class="text-lg font-bold text-white flex items-center gap-2">
                        <span>📋</span>
                        Detail Peta Geotagging
                    </h3>
                </div>
                <div class="p-6">
                    <!-- Header Info -->
                    <div class="mb-6 pb-6 border-b border-gray-200">
                        <h2 class="text-2xl font-bold text-gray-900 mb-3">{{ $petaGeotagging->judul }}</h2>
                        <div class="flex flex-wrap items-center gap-4 text-sm">
                            <div class="flex items-center gap-2 text-gray-600">
                                <span>📅</span>
                                <span>{{ $petaGeotagging->created_at->format('d M Y, H:i') }}</span>
                            </div>
                            <div class="flex items-center gap-2">
                                {!! $petaGeotagging->status_badge !!}
                            </div>
                            <div class="flex items-center gap-2 text-gray-600">
                                <span>📎</span>
                                <span>{{ $petaGeotagging->file_count }} File PDF</span>
                            </div>
                            <div class="flex items-center gap-2 text-gray-600">
                                <span>💾</span>
                                <span>{{ $petaGeotagging->formatted_total_size }}</span>
                            </div>
                        </div>
                    </div>

                    <!-- Keterangan -->
                    @if($petaGeotagging->keterangan)
                    <div class="mb-6">
                    ...
                    <!-- PDF Preview -->
                    @if(!empty($petaGeotagging->files) && is_array($petaGeotagging->files) && count($petaGeotagging->files) > 0)
                    <div>
                        <h4 class="font-semibold text-gray-700 mb-3 flex items-center gap-2">
                            <span>👁️</span>
                            Preview PDF
                        ...
                        <div class="border border-gray-300 rounded-lg overflow-hidden bg-gray-100">
                            <iframe src="{{ Storage::url($petaGeotagging->files[0]['path']) }}#toolbar=0" 
        ...
        <div class="space-y-6">
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="font-semibold text-gray-900">{{ $petaGeotagging->kelompok->nama_kelompok }}</p>
                        </div>
                        <div>
                            <label class="text-sm text-gray-500 block mb-1">Pengelola:</label>
                            <p class="font-semibold text-gray-900">{{ $petaGeotagging->user->name }}</p>
                        </div>
                        <div>
                            <label class="text-sm text-gray-500 block mb-1">Desa:</label>
                            <p class="text-gray-900">{{ $petaGeotagging->kelompok->desa }}</p>
                        </div>
                        <div>
                            <label class="text-sm text-gray-500 block mb-1">Kecamatan:</label>
                            <p class="text-gray-900">{{ $petaGeotagging->kelompok->kecamatan }}</p>
                        </div>
                        <div>
                            <label class="text-sm text-gray-500 block mb-1">Kabupaten:</label>
                            <p class="text-gray-900">{{ $petaGeotagging->kelompok->kabupaten }}</p>
                        </div>
                        <div class="pt-4 border-t border-gray-200">
                            <label class="text-sm text-gray-500 block mb-1">Kontak:</label>
                            <p class="font-medium text-gray-900">{{ $petaGeotagging->kelompok->kontak }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Form Verifikasi -->
            @if($petaGeotagging->status === 'pending')
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="p-6">
                    <form action="{{ route('bpdas.peta-geotagging.verifikasi', $petaGeotagging) }}" 
                          method="POST"
                          id="verificationForm">
                        @csrf
                        @method('PUT')
                                ...
                                <label class="flex items-center p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-green-50 hover:border-green-200 transition-all has-[:checked]:border-green-500 has-[:checked]:bg-green-50">
                                    ...
                                    <div class="ml-3 flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">Peta geotagging memenuhi syarat</p>
                                        </div>
                                    </div>
                                </label>
                                <label class="flex items-center p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:bg-red-50 hover:border-red-200 transition-all has-[:checked]:border-red-500 has-[:checked]:bg-red-50">
                                    ...
                                    <div class="ml-3 flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">Peta geotagging tidak memenuhi syarat</p>
                                        </div>
                                    </div>
                                </label>
                            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="p-6 text-center">
                    ...
                    <p class="text-sm text-gray-600 mb-4">
                        Peta geotagging ini sudah diverifikasi pada 
                        {{ $petaGeotagging->verified_at->format('d M Y') }}
                    </p>
                    <div class="mt-4">
                        {!! $petaGeotagging->status_badge !!}
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (!confirm(`Yakin ingin ${action} peta geotagging ini?`)) {
                e.preventDefault();
            }
        });
    }
