...
@section('title', 'Detail Peta Geotagging - Sistem KBR')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4">
                <h2 class="text-xl font-bold text-white">{{ $petaGeotagging->judul }}</h2>
            </div>
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="font-semibold text-gray-900">{{ $petaGeotagging->kelompok->nama_kelompok }}</span>
                </div>

                <!-- Tanggal Upload -->
                <div class="flex items-center justify-between">
                    <span class="text-gray-600 font-medium">Tanggal Upload:</span>
                    <span class="text-gray-900">{{ $petaGeotagging->created_at->format('d M Y, H:i') }}</span>
                </div>

                <!-- File Count -->
                <div class="flex items-center justify-between">
                    <span class="text-gray-600 font-medium">Jumlah File:</span>
                    <span class="font-semibold text-gray-900">{{ $petaGeotagging->file_count }} PDF</span>
                </div>

                <!-- Total Size -->
                <div class="flex items-center justify-between">
                    <span class="text-gray-600 font-medium">Total Ukuran:</span>
                    <span class="font-semibold text-gray-900">{{ $petaGeotagging->formatted_total_size }}</span>
                </div>
                ...
                <div class="pt-4 border-t">
                    ...
                    <p class="text-gray-700 bg-gray-50 p-4 rounded-lg">{{ $petaGeotagging->keterangan }}</p>
                </div>
                @endif

                <!-- Catatan BPDAS -->
                @if($petaGeotagging->status === 'ditolak' && $petaGeotagging->catatan_bpdas)
                <div class="pt-4 border-t">
                    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-lg">
                        <p class="font-medium text-red-800 mb-2">Alasan Penolakan:</p>
                        <p class="text-red-700">{{ $petaGeotagging->catatan_bpdas }}</p>
                    </div>
                </div>
                @endif

                @if($petaGeotagging->status === 'diterima' && $petaGeotagging->verified_at)
                <div class="pt-4 border-t">
                    <div class="bg-green-50 border-l-4 border-green-500 p-4 rounded-lg">
                        <p class="font-medium text-green-800 mb-1">✅ Diverifikasi oleh BPDAS</p>
                        <p class="text-sm text-green-700">
                            {{ $petaGeotagging->verified_at->format('d M Y, H:i') }}
                        </p>
                    </div>
                </div>
                @endif
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-4">
                <h3 class="text-lg font-bold text-white flex items-center gap-2">
                    <span>📎</span>
                    Daftar File PDF ({{ $petaGeotagging->file_count }})
                </h3>
            </div>

            <div class="p-6">
                <div class="space-y-3">
                    @foreach($petaGeotagging->files as $index => $file)
                ...
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <button onclick="downloadAllFiles()" 
                        ...
                        Download Semua File ({{ $petaGeotagging->file_count }} PDF)
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="mt-6 flex gap-3">
            ...
            <a href="{{ route('kelompok.peta-geotagging.edit', $petaGeotagging) }}" 
...
function downloadAllFiles() {
    const files = @json($petaGeotagging->files);
